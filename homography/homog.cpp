...
void mouse_callback(int event, int x, int y, int flags, void* userdata) {
    if (event == cv::EVENT_LBUTTONDOWN) {
        ...
        cv::circle(im_input, cv::Point(x, y), 3, cv::Scalar(0, 0, 255), cv::FILLED);
...
int main(int argc, char** argv)
{
    ...
    cv::CommandLineParser parser(argc, argv, "{@input | ../data/opencv3_scene.png | input image}"
                                                   "{@output | ../data/opencv3_model.png | output image}");
...
//    cv::waitKey(0);
    ...
    cv::imwrite(parser.get<cv::String>("@output"), im_output);
